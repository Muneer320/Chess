...
def draw_move_list():
    ...
    # Calculate the maximum number of moves that can be displayed
    visible_moves = (MOVE_HEIGHT - 30) // 30

    # Draw each move, accounting for scrolling
    for i in range(len(move_list)):
        if i >= scroll_offset and i < scroll_offset + visible_moves:
            move_text = move_list[i]
            # Alternate colors for move backgrounds
            bg_color = ALTERNATE_MOVE_COLORS[i % 2]
            y_offset = HEIGHT - MOVE_HEIGHT + 30 + (i - scroll_offset) * 30

            # Draw the move label
            pygame.draw.rect(screen, bg_color, pygame.Rect(MARGIN, y_offset, BOARD_SIZE, 30))

            if i % 2 == 0:  # White move
                move_number_surface = moves_number_font.render(f"{(i // 2) + 1}.", True, WHITE_MOVE_COLOR)
                move_surface = moves_font.render(move_text, True, WHITE_MOVE_COLOR)
            else:  # Black move
                move_number_surface = moves_number_font.render("", True, BLACK_MOVE_COLOR)  # No number for black move
                move_surface = moves_font.render(move_text, True, BLACK_MOVE_COLOR)
...
while running:
    ...
    draw_move_list()  # Show the moves at the bottom

    if game_over_message:
        # Draw endgame message if the game is over
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if square is not None:
                    if selected_square is None:
                        ...
                        if board.piece_at(square):  # Only select if there's a piece on the square
                            selected_square = square
                    else:
                        # Second click: make move
                        move = chess.Move(selected_square, square)
                        if is_pawn_promotion(move):
                            promotion = pawn_promotion()  # Call the promotion function
                            move = chess.Move(selected_square, square, promotion=promotion)  # Use selected promotion
                        
                        if move in board.legal_moves:  # Check if the move is legal
                            san_move = board.san(move)  # Get SAN notation before pushing the move
                            board.push(move)  # Push the move to the board
                            move_list.insert(0, san_move)  # Add move to the top of the list
